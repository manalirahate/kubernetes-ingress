...
NOTE: This mrethod only applies if using [external references](https://docs.nginx.com/nginx-app-protect/configuration/#external-references) 
If items on the external reference change but the spec of the ApPolicy remains unchanged (even when reaplying the policy), kubernetes will not detect the update.
In this case You can force-replace the resource. This will remove and readd the resource trigering a reload. For example:

```
kubectl replace appolicy -f your-poilcy-manifest.yaml --force
```
